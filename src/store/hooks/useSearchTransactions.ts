...
const useSearchTransactions = () => {
  ...
  const searchTransactions = useCallback(
    async (params: {
      ...
      const { pageNumber = 0, pageSize = 10, transactionId, projectId, startDate, endDate } = params;
      ...
      const fetchTransactions = async () => {
        try {
          ...
          setData(response.data.data); // Set the API response (pagination + content)
          setSuccess(true);
        } catch (error) {
          let errorMessage = 'An unexpected error occurred.';
          ...
          if (axios.isAxiosError(error) && error.response) {
            ...
            if (status === 600) {
              ...
              if (token) {
                await fetchTransactions(); // Retry with the refreshed token
                return;
              } else {
                errorMessage = 'Failed to refresh token.';
              }
